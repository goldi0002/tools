...
const PlanningSection = () => {
  ...
  const [notes, setNotes] = useState([
    { id: 'note-1', title: 'Weekly review', body: 'Capture wins and blockers from the week.' },
    { id: 'note-2', title: 'Next steps', body: 'Outline the first action for each priority.' }
  ]);
  ...
  const handleAddNote = () => {
    ...
    if (!trimmed) return;
    setNotes((prev) => [
      { id: `note-${Date.now()}`, title: trimmed.split('\n')[0], body: trimmed },
      ...prev
    ]);
    setNoteInput('');
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <ToolCard title="Notes Hub" description="Collect ideas, wins, and next steps in one place.">
        <textarea
          className="input-field min-h-[120px]"
          placeholder="Write a quick note or next step..."
          value={noteInput}
          onChange={(event) => setNoteInput(event.target.value)}
        />
        <div className="mt-4 flex flex-wrap gap-3">
          <button type="button" onClick={handleAddNote} className="btn-primary">
            Save Note
          </button>
          <button type="button" onClick={() => setNoteInput('')} className="btn-secondary">
            Clear
          </button>
        </div>
        <div className="mt-5 space-y-3">
          {notes.map((note) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h4 className="text-sm font-semibold text-slate-900 dark:text-white">{note.title}</h4>
